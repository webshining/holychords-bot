...
class UserMiddleware(BaseMiddleware):
    ...
    async def handle_message(message: Message, session, data: Dict[str, Any]):
        ...
        if not user:
            ...
            user = await User.update(
                id=message.from_user.id, name=message.from_user.full_name, username=message.from_user.username, session=session
            )
        data["user"] = user
    ...
    async def handle_callback_query(call: CallbackQuery, session, data: Dict[str, Any]):
        ...
        if not user:
            user = await User.create(
                id=call.from_user.id, name=call.from_user.full_name, username=call.from_user.username, session=session, history=[], songs=[]
            )
        else:
            user = await User.update(id=call.from_user.id, name=call.from_user.full_name, username=call.from_user.username, session=session)
        data["user"] = user
    ...
    async def handle_inline_query(query: InlineQuery, session, data: Dict[str, Any]):
        ...
        if not user:
            ...
            user = await User.update(
                id=query.from_user.id, name=query.from_user.full_name, username=query.from_user.username, session=session
            )
        data["user"] = user
