...
async def library_(message: Message, user: User, songs):
    response = await songs.v1.GetLibrary(common_pb2.Empty(), metadata=[("user_id", str(user.id))])
...
async def back_to_library_(call: CallbackQuery, user: User, songs):
    response = await songs.v1.GetLibrary(common_pb2.Empty(), metadata=[("user_id", str(user.id))])
